...
```bash
(dlv) break ./demo1.go:10
Breakpoint 1 set at 0x104fbc1e8 for main.demo1() ./demo1.go:10
```
...
```go
(dlv) continue
> main.demo1() ./demo1.go:10 (hits goroutine(1):1 total:1) (PC: 0x104fbc1e8)
     5:         "fmt"
     6: )
     7: 
     8: func demo1() {
...
```assembly
(dlv) si
> main.demo1() ./demo1.go:10 (PC: 0x104fbc1ec)
        demo1.go:8      0x104fbc1d8     fd2300d1        SUB $8, RSP, R29
        demo1.go:9      0x104fbc1dc     ff3f00f9        MOVD ZR, 120(RSP)
        demo1.go:9      0x104fbc1e0     ff7f08a9        STP (ZR, ZR), 128(RSP)
        demo1.go:10     0x104fbc1e4     01000014        JMP 1(PC)
        demo1.go:10     0x104fbc1e8*    e0031faa        MOVD ZR, R0
=>      demo1.go:10     0x104fbc1ec     e10340b2        ORR $1, ZR, R1
        demo1.go:10     0x104fbc1f0     e2031faa        MOVD ZR, R2
        demo1.go:10     0x104fbc1f4     e30301aa        MOVD R1, R3
        demo1.go:10     0x104fbc1f8     240300d0        ADRP 417792(PC), R4
        demo1.go:10     0x104fbc1fc     84002c91        ADD $2816, R4, R4
        demo1.go:10     0x104fbc200     2c20f697        CALL runtime.growslice(SB)
```
...
Breakpoint 2 set at 0x104d442bc for runtime.growslice() /usr/local/go/src/runtime/slice.go:157
```
...
```go
(dlv) continue
> runtime.growslice() /usr/local/go/src/runtime/slice.go:157 (hits goroutine(1):1 total:1) (PC: 0x104d442bc)
...
下面放另一个例子看下上述命令的用法，先放源代码。这段代码起了2个协程，爬取指定网站计算耗时，最后打印耗时：
...
这个样例演示一下分析 core dump 转储文件。这个样例中的源代码起了多个协程并发读写一个 `string` 变量，代码如下：
...
然后需要在服务器上配置一下，在程序崩溃时生成 core 文件：
...
[signal SIGSEGV: segmentation violation code=0x1 addr=0x0 pc=0x46a2bc]

goroutine 1 [running]:
runtime.throw({0x60da7f?, 0x42a11f?})
	/usr/lib/golang/src/runtime/panic.go:992 +0x71 fp=0xc000121e48 sp=0xc000121e18 pc=0x43a171
runtime.sigpanic()
	/usr/lib/golang/src/runtime/signal_unix.go:802 +0x225 fp=0xc000121e78 sp=0xc000121e48 pc=0x44fe25
runtime.memmove()
	/usr/lib/golang/src/runtime/memmove_amd64.s:149 +0xfc fp=0xc000121e80 sp=0xc000121e78 pc=0x46a2bc
runtime.recordForPanic({0x0, 0x2, 0x2})
	/usr/lib/golang/src/runtime/print.go:46 +0x10e fp=0xc000121eb8 sp=0xc000121e80 pc=0x43b8ce
runtime.gwrite({0x0, 0x2, 0x2})
	/usr/lib/golang/src/runtime/print.go:90 +0x3d fp=0xc000121ef0 sp=0xc000121eb8 pc=0x43ba5d
runtime.printstring({0x0?, 0x4e20?})
	/usr/lib/golang/src/runtime/print.go:247 +0x25 fp=0xc000121f18 sp=0xc000121ef0 pc=0x43c265
main.demo3()
	/workspace/go/src/github.com/wtifs/project-diva/src/myGo/dlv/demo3.go:23 +0x1b6 fp=0xc000121f70 sp=0xc000121f18 pc=0x5ccf56
main.main()
	/workspace/go/src/github.com/wtifs/project-diva/src/myGo/dlv/main.go:6 +0x17 fp=0xc000121f80 sp=0xc000121f70 pc=0x5cd0b7
runtime.main()
	/usr/lib/golang/src/runtime/proc.go:250 +0x1d8 fp=0xc000121fe0 sp=0xc000121f80 pc=0x43c7f8
runtime.goexit()
	/usr/lib/golang/src/runtime/asm_amd64.s:1571 +0x1 fp=0xc000121fe8 sp=0xc000121fe0 pc=0x469361
```
...
-rw------- 1 root root 90181632 5月  23 17:47 core.9029

dlv core dlv ./core.9029

# 查看栈
(dlv) bt -full
 0  0x000000000046aca1 in runtime.raise
    at /usr/lib/golang/src/runtime/sys_linux_amd64.s:168

 1  0x0000000000467140 in runtime.systemstack_switch
 ...
 4  0x000000000044fe25 in runtime.sigpanic
    at /usr/lib/golang/src/runtime/signal_unix.go:802
        g = (*runtime.g)(0xc0000021a0)

 5  0x000000000046a2bc in runtime.memmove
 ...
 9  0x00000000005ccf56 in main.demo3
    at ./demo3.go:23
        s = "bc"
        concurrency = 10000
        wg = (*sync.WaitGroup)(0xc000018300)
        i = 5756

10  0x00000000005cd0b7 in main.main
    at ./main.go:6

11  0x000000000043c7f8 in runtime.main
    at /usr/lib/golang/src/runtime/proc.go:250
        g = (*runtime.g)(0xc0000021a0)
        needUnlock = false
        fn = (unreadable empty OP stack)

12  0x0000000000469361 in runtime.goexit
```

可以看到在第 9 帧时，s 的值还是 `bc`，使用 `frame 9` 命令进入第 9 帧：

```bash
(dlv) frame 9
> runtime.raise() /usr/lib/golang/src/runtime/sys_linux_amd64.s:168 (PC: 0x46aca1)
Warning: debugging optimized function 
# 从上方的 Warning 可以看到尽管使用了 `-gcflags="all=-N -l"` 参数编译文件，但有些优化并未被关闭，导致有些变量调试不出值
Frame 9: ./demo3.go:23 (PC: 5ccf56)
    18:			}()
    19:			go func() {
    20:				s = "bc"
    21:				wg.Done()
...
到这一步可以发现原本值为 `bc` 的 `s` 变量，转成 `[]byte` 结构传到 `gwrite` 方法后长度变为了 0。长度为 0 时应在 `87` 行就进入返回分支的，实际上没有，说明 `b` 的内容在第 `87` 行后发生了修改
...
打印 `b` 的值和长度可以看出，切片 `b` 的长度为 2，但引用的数据对象确是空值，导致后续读切片时出错，触发 `panic`
...
实践中，由于生产环境往往和本机环境是隔离的，都有个跳板机，需要配置跳板机做转发，因此这个功能不是很常用
