...
![在这里插入图片描述](assets/gc_write_barriar.png)
...
它也有明显的缺点。因为栈上的对象在垃圾收集中也会被认为是根对象，所以为了保证内存的安全，Dijkstra 必须 **为栈上的对象增加写屏障** 或者 **在标记阶段完成重新对栈上的对象进行扫描**，这两种方法各有各的缺点，前者会大幅度增加写入指针的额外开销，后者重新扫描栈对象时需要暂停程序。`Go 1.8` 前采用的是后者；`1.8` 后采用两种屏障混合的做法 + 将新建对象直接标记为黑色的做法，就不需要重新扫描栈和暂停程序了。
...
用户程序删除了 A -> B 的引用，这时触发写屏障将 B 对象标记成灰色。（因为可能还有 C->B 的引用，所以需要保留B）

因为一旦该写屏障开始工作，它会保证开启写屏障时堆上所有对象的可达，所以也被称作快照垃圾收集（Snapshot GC）
...
![yuasa-delete-write-barrier](assets/2021-01-02-16095599123266-yuasa-delete-write-barrier.png)
...
2. 后台监控协程 `sysmon`，默认 `2 ` 分钟执行一次 `forcegchelper`
...
1. 标记准备（期间开启 STW、写屏障、扫描根节点）
2. 并发标记（不用 STW，可以和用户程序交替或并发调度执行，减少 STW 时间）
3. 标记结束（期间开启 STW、结束写屏障）
4. 清除：触发内存回收
...
## 第二阶段：并发标记 gcBgMarkStartWorkers()
...
这些 worker 在被唤醒后，执行的是 `gcDrain()` 函数，这个函数是用于扫描和标记堆内存中对象的核心方法，非常复杂
...
期间会让一些协程暂停工作，转而来辅助执行标记工作（比例不超过 `25%`，如 4核 CPU 就使用 1核来辅助）。辅助 `GC` 是消费者，从全局队列里取对象出来消费。
