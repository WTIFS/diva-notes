...
当我们想要向 Channel 发送数据时，就需要使用 `ch <- i` 语句，编译器会将它解析成 `OSEND` 节点并在 `cmd/compile/internal/gc.walkexpr` 中转换成 `runtime.chansend1` 函数，`chansend1` 实际调用 `chansend` 函数，其逻辑如下：
...
4. 如果缓冲区已经满了，则
    1. 将协程的状态设置为 `waiting`，移除协程与 `M` 的联系
    2. 将协程包装成 `sudog` 链表节点结构，数据保存到 `sudog.elem` 字段，然后将节点追加到 `sendq` 链表中
5. 写入完成释放锁
...
func chansend(c *hchan, ep unsafe.Pointer, block bool, callerpc uintptr) bool {
    ...
    // 2. 如果等待队列里有等待的读者，直接把数据写到它的栈里；并在下次调度时唤醒它
...
func chanrecv(c *hchan, ep unsafe.Pointer, block bool) (selected, received bool) {
    // 1. 判断可否快速返回
    if c == nil {          
        if !block {  // select 模式下空 channel 直接返回；否则进入阻塞
            return
        }
        gopark(nil, nil, waitReasonChanReceiveNilChan, traceEvGoStop, 2) // 非 select 则休眠协程，进入阻塞
        throw("unreachable")
    }
