...
Goroutine 是 Go 语言调度器中待执行的任务，它在运行时调度器中的地位与线程在操作系统中差不多，但是它的内存消耗、创建与销毀（线程需切换内核态，协程不用）、上下文切换（协程只需保存少量栈指针、计数器）代价都比线程小得多。
...
- 程序启动后，会给每个逻辑 `CPU` 分配一个 `P`；同时，会给每个 `P` 分配一个 `M`，这些 `M` 由操作系统来调度。
...
`gfget` 函数会尝试从 `P` 的本地队列里找空闲 `G`，如果本地队列为空而全局队列不为空，则从全局队列里取32个空闲 `G` 挪到 `P` 的本地队列里再取出
