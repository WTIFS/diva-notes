...
程序启动部分的代码是用汇编写的，这里以 `amd64` CPU 为例，程序的入口函数为 `runtime/asm_amd64.go/runtime·rt0_go`。里面依次执行如下步骤：

1. 初始化 `g0、m0`
1. 调用 `osinit()` 初始化系统核心数
1. 调用 `schedinit()` 初始化调度器
   1. 该函数主要会创建一批 `P`，数量默认为 `CPU` 数
   1. 如果用户设置了 `GOMAXPROCS` 环境变量，则 `P` 的数量为 `max(GOMAXPROCS, 256)`，也就是最多 256
   1. 这些 `P` 初始创建好后都放置在全局变量 `Sched ` 的 `pidle` 队列里

1. 调用 `newproc()` 函数创建出第一个 `G`，这个 `goroutine` 将执行的函数是 `runtime/proc.go/main()`
   1. 里面创建了一个新的内核线程 `M`: 系统监控 `sysmon`
   1. 初始化、启动垃圾回收
   1. 运行我们写的 `main()` 函数

1. 调用 `mstart()` 创建 `M` 线程、启动调度
...
TEXT runtime·rt0_go(SB),NOSPLIT|TOPFRAME,$0
    ...
    MOVL	24(SP), AX		// copy argc 参数个数
    MOVL	AX, 0(SP)
    MOVQ	32(SP), AX		// copy argv 函数地址及其参数
    MOVQ	AX, 8(SP)
    ...
    PUSHQ	AX
    // 创建 G 执行 runtime/proc.go/main()
    CALL	runtime·newproc(SB)
    POPQ	AX
    
    // start this M 运行刚刚创建的 goroutine
...
func main() {
    ...
    fn := main_main // 我们写的 main.go/main() 函数
    fn()
}
