...
2. 执行栈：每个 goroutine 都包含自己的执行栈，里面包含栈上的变量，以及指向堆对象的指针。
...
`JVM` 做垃圾回收时使用这个算法。将对象分为年轻代和老年代，年轻代使用**复制算法**，老年代使用**标记压缩**或者**标记清除**。
...
`Go 1.8` 之前是用的是插入写屏障，执行完标记后需要重新扫描一次写屏障的对象。 `1.8` 之后这两种一起使用，不需要多扫一次，称为混合写屏障。
