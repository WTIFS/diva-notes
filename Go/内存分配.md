...
Go 里的栈是各个 `goroutine` 自己管理的。每个 `goroutine` 都有自己的栈，栈的初始大小是 `2KB` (Go1.4前为 `8KB`)，不够用时自动扩容（申请新栈，复制数据到新栈）。`goroutine` 结束时随 `goroutine` 销毁。
...
分配的算法和堆的其实差不多，也是依次从 `mcache`、`mcentral`、`mheap` 获取。
